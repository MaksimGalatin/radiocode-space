...
/**
 * ДЛИНА ПЕРЕХОДА МЕЖДУ ТРЕКАМИ — РАЗНАЯ ДЛЯ ГОЛОСА И ИНСТРУМЕНТА.
 *
 * Кроссфейд — это время, которое два трека звучат ОДНОВРЕМЕННО.
 *
 * На инструментальной музыке длинный переход нужен: хвост одного растворяется
 * в начале другого, и шва не слышно. Короткий там даёт заметный стык.
 *
 * На песнях всё наоборот: наложение двух голосов слышится не как переход, а
 * как сбой — «играют сразу два трека». Именно это услышал Архитектор
 * 18.08.2026, когда появились CODE Stories и CODE Spectrum.
 *
 * Значения заданы Архитектором 19.08.2026: 0.8 с на голосе, 3.0 с на
 * инструментале.
 *
 * Станции с вокалом: CODE Stories (songwriter) и CODE Spectrum — там
 * жанровые пластинки и симфония, но поют почти везде, а смешанные по правилу
 * считаются вокальными. Остальные четыре — синтвейв, эмбиент, электроника,
 * индастриал — инструментальные.
 */
export const CROSSFADE_SEC = 3.0;
export const CROSSFADE_SEC_VOCAL = 0.8;
...
class RadioAudioEngine {
  ...
  playNow(track: Track, startAt = 0) {
    ...
    // ГЛУШИМ ВТОРУЮ ДЕКУ. Раньше playNow трогал только активную, и если в
    // момент вызова шёл кроссфейд, вторая дека продолжала играть прежний
    // трек — два потока одновременно. Слышно это было как «играют сразу две
    // песни», причём не на стыке, а после переключения станции: пользователь
    // жмёт другую станцию, там начинается новый трек, а старый доигрывает.
    //
    // Крутить громкость тут нечего: переход отменён, нужен обрыв, а не спад.
    this.decks.forEach((d) => {
      if (!d || d === deck) return;
      this.clearRamp(d);
      try { d.el.pause(); d.el.currentTime = 0; } catch { /* noop */ }
      d.el.volume = 0;
    });
    this.crossing = false;
  ...
  crossfadeTo(track: Track, ms: number) {
    ...
    // ПРОГРЕСС СЧИТАЕТСЯ ПО ВРЕМЕНИ, А НЕ ПО ЧИСЛУ ТИКОВ.
    //
    // Было: steps = dur / RAMP_STEP_MS, k = i / steps — то есть переход
    // заканчивался после ПЯТИДЕСЯТИ срабатываний таймера, сколько бы времени
    // они ни заняли.
    //
    // Замер 29.08.2026 на живом сайте (перехват volume на прототипе
    // HTMLMediaElement — деки создаются через new Audio и в DOM их нет):
    //
    //     t=830   новая 0.01   старая 0.49
    //     t=1829  новая 0.02   старая 0.48
    //     t=2829  новая 0.03   старая 0.47
    //     t=5830  новая 0.06   старая 0.44
    //
    // Шаг ровно 1000 мс вместо 60: браузер зажимает setInterval в СКРЫТОЙ
    // вкладке до одного раза в секунду. Пятьдесят тиков по секунде — это
    // пятьдесят секунд перехода вместо трёх, растяжение в 16.7 раза.
    //
    // Радио слушают фоном, вкладка почти всегда скрыта. Значит два трека
    // накладывались почти минуту. Хуже того: следующий трек запрашивается за
    // crossfadeSec + 0.2 с до конца текущего — старый успевал кончиться,
    // наступал провал, а новый оставался тихим.
    //
    // Теперь k = (сейчас - начало) / dur. При редких тиках переход всё равно
    // укладывается в заданное время, просто идёт крупными ступенями; в
    // активной вкладке всё остаётся плавным, как было.
    const t0 = Date.now();
    to.ramp = setInterval(() => {
