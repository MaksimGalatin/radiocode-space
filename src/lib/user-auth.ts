 ...
 *
 * ИСПРАВЛЕНО 22.08.2026 — ранний пропуск нулевого поколения был дырой.
 *
 * Было: `if (вТокене === 0) return почта;` — токен с нулём уходил мимо сверки
 * с базой. То есть человек нажимал «выйти со всех устройств», поколение в базе
 * росло до единицы, а его прежний токен с нулём продолжал работать до конца
 * срока — все 30 суток. Отзыв сессии на этом сайте попросту не действовал для
 * токенов, выданных до 19.08.2026.
 *
 * Опасение, ради которого делался пропуск («строгая сверка разлогинит всех
 * разом»), оказалось необоснованным, и это проверяется рассуждением, а не
 * надеждой. Сравнение идёт не «равно», а «в базе БОЛЬШЕ, чем в токене»:
 *   база 0, токен 0  → 0 > 0 ложно → пускаем. Человек, который ни разу не
 *                      выходил и не менял пароль, остаётся в кабинете.
 *   база 1, токен 0  → 1 > 0 истинно → отзываем. Ровно тот, кто САМ попросил
 *                      выйти со всех устройств или сменил пароль.
 * Массового разлогина не происходит: выбрасывает только тех, кто сам этого
 * потребовал. Ровно так с 20.08.2026 работают central, works и eternal —
 * раздел 9 требует одинакового поведения на всех четырёх сайтах.
 */
...
export async function сессияДействительна(req: NextRequest): Promise<string | null> {
  ...
  try {
    ...
    // Строго как на трёх остальных сайтах: отзываем, когда в базе поколение
    // ВЫШЕ, чем в токене. Равенство и нули пропускаем.
    return вБазе > вТокене ? null : почта;
  } catch {
...
/**
 * Синоним `сессияДействительна` под именем, которое используют ручки.
 *
 * ПОЧЕМУ ОН ПОЯВИЛСЯ 06.09.2026. Проверка типов на радио падала дважды:
 * `src/app/api/scan/route.ts`, строки 963 и 1004, импортируют
 * `getFreshSessionEmail` из этого файла, а такой функции здесь не было.
 * Замер по четырём сайтам: функция есть на aifa.works, центральном и
 * code-eternal и отсутствовала ТОЛЬКО на радио — то есть ручка скана
 * здесь падала на импорте.
 *
 * Логика не дублируется: на радио она уже написана как
