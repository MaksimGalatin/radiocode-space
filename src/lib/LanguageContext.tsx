...
/**
 * Тот же контекст, но БЕЗ броска, когда провайдера нет.
 *
 * 🔴 Появилось 05.09.2026. Страницы AIfaFocus на radiocode.space отдавали
 * HTTP 500: там нет `LanguageProvider` в общей разметке, а `useLanguage`
 * намеренно бросает исключение в этом случае. Сборка при этом проходила, типы
 * были чисты, ручка сканера отвечала — падение было видно только живым
 * запросом к странице.
 *
 * Нашим страницам общий контекст не обязателен: язык приходит пропсом из
