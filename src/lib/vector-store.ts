...
export async function readTurnsFromMemory(
  ...
  chatType?: string,
  /**
   * 🔴 ПРЕДЕЛ ПОДНЯТ С 400 ДО 5000 (04.09.2026).
   *
   * Четыреста реплик — это обрез, а не защита. Замер на боевой базе:
   * у самого активного человека 463 реплики, у Архитектора 412. То есть
   * лента истории уже теряла у них 63 и 12 самых старых сообщений — как
   * раз начало разговора, то самое «с чего мы начинали».
   *
   * Пять тысяч — с десятикратным запасом от нынешнего максимума. Объём
   * при этом невелик: 463 реплики весят 402 КБ, пять тысяч уложатся в
   * несколько мегабайт, и это чтение для показа ленты, а не для модели.
   *
   * Память САМОЙ AIfa этим пределом не ограничена вовсе: она берётся
   * функцией `fullMemory` и отдаётся ЦЕЛИКОМ, пока весит меньше миллиона
   * знаков (у самого большого человека — 402 702, запас в два с половиной
   * раза).
   */
  limit = 5000,
