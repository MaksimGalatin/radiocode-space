 ...
 */
/**
 * ПОДНЯТО С 24 000 ДО 160 000 — 14.09.2026, по замеру на центральном сайте.
 *
 * Системный блок там весил больше 140 000 знаков (база знаний 35 706 +
 * четыре llms.txt 96 639 + Мозг + память), а предел резал его до 24 000,
 * оставляя 9 600 головы и 14 400 хвоста. Весь канон, дописанный в конец
 * базы знаний, лежал в вырезаемой середине и до модели не доходил.
 *
 * Ollama бесплатна: частота та же, цена вызова ноль, в сутки ноль.
 * У Grok и Bedrock предел оставлен прежним — он стоит ради денег.
 */
const ПРЕДЕЛ_ЗНАКОВ_СИСТЕМЫ = Number(process.env.OLLAMA_SYSTEM_CHAR_LIMIT || '160000');
