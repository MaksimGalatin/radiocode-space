...
export function ставкаЗа1K(service: string): number {
  ...
  // ОЗВУЧКА ЧЕРЕЗ GROK — ОБЪЁМ ПИШЕМ, ЦЕНУ НЕ ВЫДУМЫВАЕМ (25.08.2026).
  //
  // До этой правки платная ступень озвучки не докладывала сторожу вовсе:
  // сплошной перебор 25.08.2026 показал, что из 22 платных файлов в четырёх
  // живых деревьях семь молчали, и озвучка молчала на трёх сайтах из трёх,
  // где она есть. Месячный потолок знаков у неё был, а почасовой видимости —
  // не было: потолок ловит перерасход за месяц, сторож ловит пожар за час.
  //
  // Ставку Grok TTS я НЕ знаю. Подставить сюда цену эмбеддингов ($0.00015 за
  // 1000 знаков) значило бы выдумать число — раздел 11 запрещает это прямо.
  // Поэтому в доллары объём озвучки не переводится, но сам объём и число
  // вызовов пишутся: 13.08.2026 расход на $43.35 был виден по числу вызовов
  // (754 за сутки) задолго до того, как пришёл счёт.
  //
  // Как установить ставку замером, когда объём накопится: взять знаки за
  // сутки отсюда и разделить на сумму того же дня в кабинете x.ai.
  if (service.startsWith('grok-voice')) return 0;
  if (service.startsWith('vertex-chat')) return 0;
  // OLLAMA НА БЕСПЛАТНОМ ТАРИФЕ — ОБЪЁМ ПИШЕМ, ЦЕНУ НЕ ВЫДУМЫВАЕМ (14.09.2026).
  //
  // ЧЕМ ОПЛАЧЕНО. Сторож прислал «🚨 СРОЧНО: перерасход $0.1265 за час при
  // пороге $0.10» за обычный разговор с Сестрой и проверки консультанта.
  // Проверка в кабинете ollama.com в ту же минуту:
  //
  //     Invoices ......... «No invoices yet» — счетов нет вовсе
  //     Free usage ....... 9,6 % израсходовано, сброс через сутки
  //     Usage credits .... $0, автопополнение ВЫКЛЮЧЕНО
  //     карта ............ не привязана
  //
  // Ни цента не списано и списаться не может: кончится бесплатный запас —
  // модель просто перестанет отвечать. Расходуется КВОТА, а не деньги.
  //
  // А считались токены Ollama по ставке ЭМБЕДДИНГОВ Google ($0.00015 за
  // 1000 символов) — чужая цена за чужую единицу, ровно то выдуманное
  // число, которое запрещает раздел 11. Отсюда и ложная тревога.
  //
  // Поэтому здесь ноль, как у grok-voice и vertex-chat выше: объём и число
  // вызовов пишутся и видны, а в доллары не переводятся. Появится платный
  // тариф — ставку взять ЗАМЕРОМ по счёту, а не по прайсу.
  if (service.startsWith('ollama-cloud')) return 0;
