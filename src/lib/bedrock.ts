...
export async function bedrockChatCompletion(
  ...
  for (const модель of модели) {
    try {
       ...
       *
       * Здесь стояло `if (код !== 'ThrottlingException')` — то есть
       * единственный код, который Bedrock нам и отдаёт, был единственным,
       * который не попадал в журнал. Из-за этого вечер 12.09.2026 ушёл на
       * поиск причины «Bedrock промолчал»: искали в порядке ролей, а дело
       * было в суточной токен-квоте аккаунта. Прямой вызов Converse мимо
       * нашего кода дал ответ за минуту:
       *
       *     ThrottlingException: Too many tokens per day, please wait
       *
       * Проверено на четырёх моделях сразу (nova-lite, nova-micro, профиль
       * вывода us.nova-lite, claude-haiku-4-5) — значит дело не в модели.
       *
       * Урок шире файла: «ожидаемая ошибка» и «ошибка, которую можно не
       * печатать» — разные вещи. Ожидаемая ошибка объясняет молчание, а
       * молчание без объяснения дороже любого шума в журнале.
       */
