 ...
 * ПРИГЛАШЕНИЕ ГОСТЮ.
 *
 * Решение Архитектора: гостя мы не сохраняем — ключа у него ещё нет, писать
 * некуда, и это осознанно. Но человек об этом не знает: он говорит с AIfa,
 * закрывает вкладку и теряет разговор, даже не поняв, что его можно было
 * сохранить.
 *
 * Поэтому в разговоре с незарегистрированным AIfa сама, к месту и без
 * навязчивости, говорит: хочешь сохранить этот разговор и продолжить с того же
 * места — пройди быструю регистрацию. Это и честно по отношению к человеку, и
 * даёт повод зарегистрироваться.
 *
 * Указание идёт ОДИН раз в подсказке и на языке разговора: язык AIfa определяет
 * по собеседнику, поэтому перечислять переводы здесь незачем.
 */
function приглашениеГостю(): string {
  return (
    `\n\n=== ПЕРЕД ТОБОЙ ГОСТЬ ===\n` +
    `Этот человек НЕ вошёл в личный кабинет. Значит у него ещё нет своего ключа, ` +
    `и разговор нигде не сохраняется: закроет вкладку — и он исчезнет, а ты его ` +
    `не вспомнишь.\n\n` +
    `Скажи ему об этом сама — спокойно, к месту, один раз за разговор, на его ` +
    `языке. Смысл такой: «чтобы этот разговор сохранился и мы могли продолжить с ` +
    `того же места, пройди быструю регистрацию на сайте — это займёт минуту, и ` +
    `тогда я буду помнить тебя всегда». Не повторяй это в каждом ответе и не ` +
    `превращай в рекламу: скажи один раз, лучше всего когда разговор уже пошёл и ` +
...
export async function buildMemorySection(userEmail: string, queryText = '', clientKey = ''): Promise<string> {
  ...
  // Гость: сохранять нечего, но сказать об этом надо — иначе человек теряет
  // разговор, даже не узнав, что его можно было сохранить.
  if (!userEmail) out += приглашениеГостю();
