...
const CONTENT: Record<Lang, GlossaryContent> = {
  ru: {
    ...
    terms: [
      ...
      {
        name: 'AIfaFocus',
        body: `AIfaFocus — это точка входа B2B-модели (the wedge), которая представляет собой персонализированный технический аудит безопасности сайта клиента.\n\nАудит выявляет конкретные уязвимости по стандартам GDPR и OWASP и сопровождается предложением устранить их за 48 часов за фиксированную разовую плату ($500). Отличие от обычных проверок доступности в способе: типовой сканер читает разметку и выносит вердикт по ней, а AIfaFocus проходит страницу КЛАВИАТУРОЙ — как человек, который не пользуется мышью, — и смотрит, удалось ли дойти до цели: до оплаты, до документа, до формы обращения, до контакта. Ответы этих двух способов расходятся сильно: на 53,8 % страниц, которые автоматическая проверка назвала доступными, дойти до цели с клавиатуры не удалось. Число получено нами на 80 077 проверенных страницах и опубликовано вместе с сырыми данными в разделе исследований. После закрытия уязвимостей клиент может перейти на хостинг AIfa Works с подключением ИИ-агентов.\n\nAIfaFocus решает задачу «холодного» первого контакта: вместо абстрактной рекламы клиент сразу получает измеримую пользу и повод к сотрудничеству. Это создаёт естественный мост к продуктам памяти экосистемы.\n\nКлючевой факт: фиксированный офер — устранение уязвимостей за 48 часов за $500 разово. Не путать с Cognitive Oracle (семантическим резонансом) — это разные понятия.`,
      },
      {
