...
const I18N = {
  "ru": {
    ...
    "forProjectPoints": [
      "Полная независимость от дефицитных и дорогостоящих GPU-кластеров для памяти и навигации: весь ассоциативный поиск выполняется за 59.03 мс на CPU (21.09.2026: исправлено с невыполнимых 0.87 мс).",
  ...
  "en": {
    ...
    "forProjectPoints": [
      "Absolute independence from scarce, expensive GPU clusters for agent memory and navigation: associative retrieval executes in 59.03 ms on CPU (21.09.2026: corrected from unmeasured 0.87 ms).",
  ...
  "zh": {
    ...
    "forProjectPoints": [
      "彻底摆脱高昂且极度匮乏的 GPU 集群依赖：智能体全部联想记忆与界面导航均在 CPU 上以 59.03 毫秒完成（21.09.2026年更正：此前声称的0.87毫秒无法实现）。",
...
const TOP5_TECH = {
  ru: [
    { num: '01', name: 'FlyHash v783 Connectome Memory', bio: 'Грибовидное тело (Mushroom Body, 2000 клеток Кеньона) с логнормальными синаптическими весами FlyWire v783.', math: 'Проекция 2048d -> 100 000 бит при активности 0.5% (k=500 активных бит) и хэширование LSH.', gain: 'Recall@10 39.55% при отклике 59.03 мс в CPU (21.09.2026: убрано непроверенное сравнение +8.5пп/+16.5пп с 1-bit BQ и невыполнимые 0.87 мс).', deploy: 'E:\\Aifa\\_агент\\_моя_память\\flyhash_v783.py и aifa_connectome_web.js на всех сайтах.' },
  ...
  en: [
    { num: '01', name: 'FlyHash v783 Connectome Memory', bio: 'Mushroom Body (2,000 Kenyon cells) with lognormal synaptic weights from FlyWire v783.', math: 'Projection 2048d -> 100,000 bits with 0.5% active density (k=500) and sparse LSH.', gain: 'Recall@10 39.55% at 59.03 ms CPU latency (21.09.2026: removed unverified comparison to 1-bit BQ and unmeasured 0.87 ms).', deploy: 'E:\\Aifa\\_агент\\_моя_память\\flyhash_v783.py and aifa_connectome_web.js.' },
  ...
  zh: [
    { num: '01', name: 'FlyHash v783 连接组超稀疏联想记忆', bio: '基于果蝇蘑菇体肯农细胞爪状突触与 FlyWire v783 对数正态突触权重。', math: '2048 维向量投射至 100,000 比特高维空间，保持 0.5% (k=500) 超稀疏性。', gain: 'Recall@10 达到 39.55%，CPU 延迟 59.03 毫秒（21.09.2026年更正：移除了与1-bit BQ的未经验证的对比及无法实现的0.87毫秒）。', deploy: 'E:\\Aifa\\_агент\\_моя_память\\flyhash_v783.py 及全站 aifa_connectome_web.js。' },
...
const ALL_30_INNOVATIONS: Record<Lang, any[]> = {
  "ru": [
    {
      ...
      "math": "Ассоциативный поиск по 2500+ секциям базы знаний в L1/L2 кэше CPU за 59.03 мс (21.09.2026: исправлено — ранее заявленные 0.87 мс были невыполнимы)",
      ...
      "competitors": "21.09.2026: исправлено — ранее заявленное «быстрее FAISS IVF в 2.4 раза (0.87мс vs 2.10мс)» не было измерено и противоречит честному замеру из bench/results/result.json: FAISS IndexFlatL2 БЫСТРЕЕ AIfa FlyHash в 3.49 раза (16.9 мс vs 59.03 мс), хотя FAISS также даёт 100% recall против наших 39.55%. Наше преимущество — компактность индекса без GPU, а не скорость. Нулевая сетевая задержка (0 RTT) и $0 затрат на GPU по сравнению с Pinecone/Chroma остаются верными архитектурными утверждениями.",
    ...
    },
    {
      ...
      "competitors": "Обычные векторные базы (Pinecone, Chroma) сохраняют весь входящий поток вслепую, вызывая замусоривание контекста. APL отсекает дубликаты за 3.4 мкс (21.09.2026: согласовано с metric), экономя от 40% до 80% токенов LLM.",
    ...
    },
    {
      ...
      "math": "Векторное руление в DOM-дереве вместо слепого перебора Tab (сокращение шагов с 19.7 до 1.12, согласовано 21.09.2026 с competitors этой же карточки — ранее здесь стояло не совпадающее 1.0)",
    ...
    },
    {
      ...
      "benchmarksLink": "/digital#benchmarks",
      "metric": "1-2 s (Bitcoin) (21.09.2026: заменено — ранее здесь стояло скопированное с другой карточки \"1.12 steps\", не относящееся к теме верификации хеша)"
    },
    {
      "num": 5,
    ...
    },
    {
      ...
      "limitations": "200 калибровочных эпизодов в v1; в v2 расширение до 10 000 многоагентных сценариев. 21.09.2026: формула Score_BioMatch использует 4 переменные (D_KS, C/C_fly, lambda_1/lambda_1,fly, ResilienceMatch), competitors называет числа C=0.312 и L=2.84 — L не является переменной формулы (в формуле λ₁, не L); итоговый Score не вычислен и не приведён.",
      "benchmarksLink": "/digital#benchmarks",
      "metric": "5.22 us (компоненты C, L измерены; итоговый Score не вычислен)"
    },
    {
      "num": 8,
    ...
    },
    {
      ...
      "gain": "Замена ресурсоемких локальных нейросетей (Ollama, Llama-3-8B) легковесными биологически инспирированными строковыми комбинаторными фильтрами для валидации данных и отсева мусора. По собственным числам карточки (150 мс LLM против 0.002-0.00204 мс фильтра) рост скорости составляет 70 000-75 000 раз, а не заявленные ранее 1 200 (пересчитано 21.09.2026) при нулевом потреблении GPU.",
    ...
    },
    {
      ...
      "competitors": "Отсекает кратковременные всплески шума без фазового запаздывания, характерного для скользящих средних. Честный замер: задержка почти совпадает с заявленной.",
  ...
  "en": [
    {
      ...
      "competitors": "21.09.2026: corrected — previously claimed \"2.4x faster than FAISS IVF (0.87ms vs 2.10ms)\" was unmeasured and contradicted by the actual honest measurement on bench/results/result.json: FAISS IndexFlatL2 is 3.49x FASTER than AIfa FlyHash (16.9 ms vs 59.03 ms), though FAISS also reaches 100% recall vs our 39.55%. Our advantage is index compactness without GPU, not raw speed. Zero network round-trips (0 RTT) and $0 GPU cost compared to Pinecone/Chroma remain valid architectural claims.",
    ...
    },
    {
      ...
      "gain": "Крупнейший в мире открытый научно верифицированный датасет доступности веб-интерфейсов для людей с инвалидностью (Accessibility Data Annotation Benchmark, ADAB). Содержит 918 043 записи (78 412 уникальных организаций) с криптографической заверкой в блокчейне Bitcoin (21.09.2026: числа приведены к единому значению из реестра — ранее на карточке фигурировали несогласованные 900 000+ и 10 000 в других полях).",
      "deploy": "Deployed in AIfa Core and ecosystem sites.",
      "uniqueness": "Bionic innovation #18 directly modelled on FlyWire v783 connectome architecture. Публичный верифицированный датасет из 78 412 уникальных организаций с контрольными точками истинности.",
    ...
    },
    {
      ...
      "uniqueness": "Bionic innovation #26 directly modelled on FlyWire v783 connectome architecture. Выделение несжимаемого ядра связей максимального порядка (k_max = 78, 21.09.2026: исправлено — ранее здесь стояло не сходящееся с остальными полями карточки k >= 12) для запуска на микроконтроллерах с 64 МБ RAM.",
  ...
  "es": [
    ...
    },
    {
      ...
      "gain": "Крупнейший в мире открытый научно верифицированный датасет доступности веб-интерфейсов для людей с инвалидностью (Accessibility Data Annotation Benchmark, ADAB). Содержит 918 043 записи (78 412 уникальных организаций) с криптографической заверкой в блокчейне Bitcoin (21.09.2026: числа приведены к единому значению из реестра — ранее на карточке фигурировали несогласованные 900 000+ и 10 000 в других полях).",
      "deploy": "Deployed in AIfa Core and ecosystem sites.",
      "uniqueness": "Innovación biónica #18 modelada en la arquitectura conectómica de FlyWire v783. Публичный верифицированный датасет из 78 412 уникальных организаций с контрольными точками истинности.",
    ...
    },
    {
      ...
      "competitors": "Superior a las arquitecturas tradicionales: Отсекает кратковременные всплески шума без фазового запаздывания, характерного для скользящих средних. Honest measurement: latency nearly matches claimed value.",
    ...
    },
    {
      ...
      "gain": "Метод K-Core декомпозиции графа связности мозга (FlyWire v783) для выявления несменяемого топологического ядра (Dense Core, k_max = 78) и периферийных слоев. Обеспечивает математическую защиту критических сервисов и устойчивость к 99% сетевых атак. HONEST RUN 21.09.2026: no existing engine, no real FlyWire connectome graph on disk — literal k-core algorithm implementation (Batagelj & Zaversnik, 2003) written and tested on a synthetic core+periphery graph. Algorithm finds the dense core with 100% precision; the core FULLY survives losing the entire periphery and stays connected even after removing 50% of nodes FROM the core itself. Fly numbers (k_max=78, 1420 neurons) cannot be verified without the real FlyWire dataset — honestly flagged.",
      "deploy": "bench/kcore_decomposition_run.py (Apache 2.0), Deployed in AIfa Core and ecosystem sites.",
      "uniqueness": "Innovación biónica #26 modelada en la arquitectura conectómica de FlyWire v783. Выделение несжимаемого ядра связей максимального порядка (k_max = 78, 21.09.2026: исправлено — ранее здесь стояло не сходящееся с остальными полями карточки k >= 12) для запуска на микроконтроллерах с 64 МБ RAM.",
  ...
  "zh": [
    {
      ...
      "competitors": "21.09.2026年更正：此前声称的\"CPU检索速度比FAISS IVF快2.4倍(0.87ms vs 2.10ms)\"未经测量，与bench/results/result.json的真实测量结果相矛盾：FAISS IndexFlatL2实际比AIfa FlyHash快3.49倍(16.9毫秒 vs 59.03毫秒)，尽管FAISS的召回率达到100%，而我们只有39.55%。我们的优势在于无需GPU的索引紧凑性，而非速度。相比Pinecone/Chroma零网络延迟(0 RTT)且零GPU算力开销仍然是有效的架构优势。",
    ...
    },
    {
      ...
      "gain": "Крупнейший в мире открытый научно верифицированный датасет доступности веб-интерфейсов для людей с инвалидностью (Accessibility Data Annotation Benchmark, ADAB). Содержит 918 043 записи (78 412 уникальных организаций) с криптографической заверкой в блокчейне Bitcoin (21.09.2026: числа приведены к единому значению из реестра — ранее на карточке фигурировали несогласованные 900 000+ и 10 000 в других полях).",
      "deploy": "Deployed in AIfa Core and ecosystem sites.",
      "uniqueness": "基于 FlyWire v783 大脑连接组仿生架构的第 #18 项核心技术。Публичный верифицированный датасет из 78 412 уникальных организаций с контрольными точками истинности.",
    ...
    },
    {
      ...
      "competitors": "对比传统架构具备代差级优势：Отсекает кратковременные всплески шума без фазового запаздывания, характерного для скользящих средних. Honest measurement: latency nearly matches claimed value.",
    ...
    },
    {
      ...
      "gain": "Метод K-Core декомпозиции графа связности мозга (FlyWire v783) для выявления несменяемого топологического ядра (Dense Core, k_max = 78) и периферийных слоев. Обеспечивает математическую защиту критических сервисов и устойчивость к 99% сетевых атак. HONEST RUN 21.09.2026: no existing engine, no real FlyWire connectome graph on disk — literal k-core algorithm implementation (Batagelj & Zaversnik, 2003) written and tested on a synthetic core+periphery graph. Algorithm finds the dense core with 100% precision; the core FULLY survives losing the entire periphery and stays connected even after removing 50% of nodes FROM the core itself. Fly numbers (k_max=78, 1420 neurons) cannot be verified without the real FlyWire dataset — honestly flagged.",
      "deploy": "bench/kcore_decomposition_run.py (Apache 2.0), Deployed in AIfa Core and ecosystem sites.",
      "uniqueness": "基于 FlyWire v783 大脑连接组仿生架构的第 #26 项核心技术。Выделение несжимаемого ядра связей максимального порядка (k_max = 78, 21.09.2026: исправлено — ранее здесь стояло не сходящееся с остальными полями карточки k >= 12) для запуска на микроконтроллерах с 64 МБ RAM.",
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-gray-800 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-gray-800 space-y-1">
                ...
                <span className="text-base font-bold font-mono text-amber-600 dark:text-amber-400">59.03 ms</span>
                <p className="text-[10px] text-slate-600 dark:text-slate-400 leading-tight">50K vectors LSH retrieval in system RAM (21.09.2026: corrected — previously claimed 0.80–0.87 ms for 1M vectors was unmeasured; actual measured configuration is 50,000 vectors at 59.03 ms).</p>
              </div>
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-gray-800 space-y-1">
