...
/**
 * ЧИСЛА ПЕРЕЗАМЕРА 10.09.2026, 21:30.
 *
 * 🔴 Прежние числа здесь (1 207 адресов, 7 924 замера, 59,4 %, 3 512
 * нарушений) были сняты в 03:59 — и устарели к вечеру того же дня, потому что
 * мы сами починили измеряемое:
 *
 *   03:32  контраст страниц исследования  (#64748b → #72839a)
 *   04:53  контраст читален               (--muted #6B6B80 → #8A8AA0)
 *   17:49  tabIndex прокручиваемым таблицам, 45 областей
 *
 * Две починки из трёх случились ПОСРЕДИ замера, шедшего с 00:44 до 03:59.
 * Опубликованное число показывало нас ХУЖЕ, чем есть.
 *
 * ЧТО ИЗМЕНИЛОСЬ НА ОДНИХ И ТЕХ ЖЕ АДРЕСАХ. Сравнение по 799 адресам,
 * измеренным оба раза (инструмент `chestnoe_sravnenie_zamerov.py`):
 *
 *   было 03:59    6 258 замеров, чистых 3 251 (51,9 %), нарушений 3 302
 *   стало 21:30   6 184 замера,  чистых 5 110 (82,6 %), нарушений 1 177
 *
 * То есть улучшение настоящее, а не следствие смены состава: +30,7 п.п. и
 * минус 2 125 нарушений на одном и том же наборе страниц.
 *
 * ЗНАМЕНАТЕЛЬ НАЗЫВАЕМ ВСЛУХ. В перезамере 2 444 замера оказались негодными
 * против 1 732 утром: наши же хостинги ограничили частоту обхода, и 310
 * страниц ответили 429 вместо 204. Доля чистых считается среди ГОДНЫХ
 * замеров. Выпавшие адреса были ЛУЧШЕ среднего (87,4 % чистых) — значит их
 * отсутствие долю не льстит, а слегка занижает.
 */
