  ...
  return (
    <div className="cab-cols2 cab-fade">
        ...
        {!d ? <div style={{ display: "grid", gap: 10 }}><Skeleton h={54} /><Skeleton h={80} /><Skeleton h={40} /></div> : (
          <>
            <div style={{ display: "flex", gap: 24, marginBottom: 14 }}>
              <div><div style={{ fontSize: 14, color: TOKENS.mut }}>{t("refInvited")}</div><div style={{ fontSize: 24, fontWeight: 800, color: TOKENS.cyan }}>{d.count}</div></div>
              <div><div style={{ fontSize: 14, color: TOKENS.mut }}>{t("refBalance")}</div><div style={{ fontSize: 24, fontWeight: 800, color: TOKENS.green }}>${Number(d.balance).toFixed(2)}</div></div>
            </div>
            ...
            <div style={{ fontSize: 14, color: TOKENS.mut, marginBottom: 8, lineHeight: 1.5 }}>{t("refLvlNote")}</div>
            {Number(d.missed) > 0 && (
              <div style={{ fontSize: 15, color: TOKENS.amber, marginBottom: 10, lineHeight: 1.5 }}>
                {t("refMissed", { m: Number(d.missed).toFixed(2) })}
              </div>
            )}
            <div style={{ fontSize: 15, color: TOKENS.mut }}>{t("refYourLink")}:</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 6, flexWrap: "wrap" }}>
              <code style={{ fontSize: 15, fontFamily: "monospace", color: TOKENS.cyan, wordBreak: "break-all", flex: "1 1 220px" }}>{refLink}</code>
              <button className="cab-btn cab-btn-ghost" onClick={copy} style={{ padding: "7px 14px", fontSize: 15 }}>{copied ? t("refCopied") : "📋 " + t("refCopy")}</button>
            </div>
            {d.list?.length > 0 && (
              <div style={{ marginTop: 16 }}>
                ...
                {d.pages > 1 && (
                  <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 10 }}>
                    <button className="cab-btn cab-btn-ghost" disabled={page <= 0} onClick={() => load(page - 1)} style={{ padding: "5px 12px", fontSize: 15 }}>←</button>
                    <span style={{ fontSize: 15, color: TOKENS.mut, alignSelf: "center" }}>{page + 1}/{d.pages}</span>
                    <button className="cab-btn cab-btn-ghost" disabled={page >= d.pages - 1} onClick={() => load(page + 1)} style={{ padding: "5px 12px", fontSize: 15 }}>→</button>
                  </div>
                )}
              </div>
            )}
      ...
      <Card>
        ...
        {msg && <div style={{ marginTop: 10, fontSize: 15, color: TOKENS.sub, textAlign: "center" }}>{msg}</div>}
      </Card>
    </div>
  );
}
