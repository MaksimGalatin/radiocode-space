    ...
    <Card style={{ borderColor: "rgba(16,185,129,0.35)" }}>
      ...
      {watches.length > 0 && (
        <div style={{ marginTop: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: TOKENS.text, marginBottom: 10 }}>{t("shieldWatchList")}</div>
          {watches.map((w) => (
            <div key={w.domain} style={{ border: "1px solid rgba(42,42,58,0.7)", borderRadius: 10, padding: 12, marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 11, color: w.verified ? TOKENS.green : TOKENS.sub }}>
                  {w.verified ? "✓" : "•"} {w.verified ? (w.method === "dns" ? "DNS" : "file") : t("shieldPending")}
                </span>
                <button className="cab-btn" onClick={() => stop(w.domain)} style={{ marginLeft: "auto", fontSize: 11, padding: "4px 10px", background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: TOKENS.sub }}>
                  {t("shieldStop")}
                </button>
              </div>
              ...
              {!w.verified && w.instructions && (
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: TOKENS.text, marginBottom: 4 }}>{t("shieldProofTitle")}</div>
                  <div style={{ fontSize: 12, color: TOKENS.sub, lineHeight: 1.6, marginBottom: 10 }}>{t("shieldProofWhy")}</div>

                  <div style={{ fontSize: 12, fontWeight: 700, color: TOKENS.text }}>{t("shieldProofDns")}</div>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 2 }}>
                    ...
                    <button className="cab-btn" onClick={() => copy(w.instructions!.dns.value, w.domain + "dns")} style={{ fontSize: 10.5, padding: "3px 8px", background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: TOKENS.sub }}>
                      {copied === w.domain + "dns" ? t("shieldCopied") : t("shieldCopy")}
                    </button>
                  </div>

                  <div style={{ fontSize: 12, fontWeight: 700, color: TOKENS.text, marginTop: 10 }}>{t("shieldProofFile")}</div>
                  <div style={{ ...mono, marginTop: 4 }}>{w.instructions.file.url}</div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 2 }}>
                    <div style={mono}>{w.instructions.file.content}</div>
                    <button className="cab-btn" onClick={() => copy(w.instructions!.file.content, w.domain + "file")} style={{ fontSize: 10.5, padding: "3px 8px", background: "transparent", border: "1px solid rgba(148,163,184,0.3)", color: TOKENS.sub }}>
                      {copied === w.domain + "file" ? t("shieldCopied") : t("shieldCopy")}
                    </button>
                  </div>

                  <button className="cab-btn" disabled={checking === w.domain} onClick={() => verify(w.domain)} style={{ marginTop: 12, background: "linear-gradient(135deg,#10B981,#059669)", fontSize: 12 }}>
