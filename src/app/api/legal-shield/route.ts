...
/**
 * Правовой Щит и наблюдение AIfaFocus живут на aifa.works — там AIfaFocus.
 *
...
async function forward(req: NextRequest, method: 'GET' | 'POST' | 'DELETE') {
  ...
  // через наш сайт долбить AIfaFocus сколько угодно, и там это выглядело бы как
  ...
  const адрес_lshield_proxy = clientIp(req as never);
  if (адрес_lshield_proxy !== 'unknown' && !(await dbRateLimit(`lshield_proxy:${адрес_lshield_proxy}`, 20, 60_000))) {
