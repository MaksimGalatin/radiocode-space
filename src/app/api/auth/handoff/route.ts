...
export async function GET(req: NextRequest) {
  ...
  // Адрес возврата вычисляется ДО щита: он нужен щиту, чтобы вернуть
  // человека на сайт вместо страницы с ошибкой.
  ...
  if (адресБилета !== 'unknown' && !(await dbRateLimit(`auth_handoff:${адресБилета}`, 40, 600000))) {
    // 🔴 ЧЕЛОВЕК ВИДЕЛ ГОЛЫЙ JSON ВМЕСТО САЙТА (починено 11.09.2026).
    //
    // Щит стоит с 09.09.2026 и работает правильно — но при срабатывании он
    // отдавал `{"error":"too_many"}` с типом application/json. А сюда
    // человек попадает НЕ РУКАМИ: на сайт-спутник он заходит обычной
    // ссылкой, и переброс за билетом делает наш же скрипт. То есть
    // посетитель вместо страницы получал строку `{"error":"too_many"}`
    // на белом фоне.
    //
    // Замер 11.09.2026, чистый браузер без куки, три сайта из четырёх:
    //     тип .............. application/json
    //     видно человеку ... {"error":"too_many"}
    //
    // Двумя десятками строк ниже — при отсутствии общего секрета — было
    // написано верно: «Молча возвращаем человека обратно». Здесь этого
    // не сделали.
    //
    // Щит НЕ ОСЛАБЛЕН: билет по-прежнему не выдаётся, перебор ничего не
    // получает. Изменилось только то, что видит человек.
    if (возврат) {
