...
export async function POST(req: Request) {
  ...
  try {
    ...
    /**
     * 🔴 СВЕРКА УПЛАЧЕННОЙ СУММЫ С СУММОЙ ЗАКАЗА. Добавлено 12.09.2026 по
     * прямому слову Архитектора: «полностью ВСЁ чини».
     *
     * ЧТО БЫЛО НЕ ТАК. Выше проверялась подпись уведомления и его статус — и
     * этого довольно, чтобы уведомление не подделали снаружи. Но СУММА не
     * сверялась ни с чем: заказ помечался оплаченным по номеру, тариф
     * выдавался по `pay_orders.tier`, а начисление амбассадорам шло от
     * `pay_orders.amount` — то есть от того, сколько ЗАПРОСИЛИ, а не сколько
     * ПРИШЛО.
     *
     * 🔴 ГЛАВНАЯ ОСТОРОЖНОСТЬ — ВАЛЮТЫ. Первая версия этой правки брала
     * максимум из четырёх полей уведомления и сравнивала с суммой заказа.
     * Так делать НЕЛЬЗЯ: `price_amount` у поставщика назван в валюте заказа
     * (доллары), а `actually_paid` и `pay_amount` — в криптовалюте. Заказ на
     * $375, оплаченный 0,0015 BTC, дал бы «пришло 0,0015 вместо 375» и
     * заблокировал бы ЧЕСТНЫЙ платёж. Поймано до выкатки.
     *
     * Поэтому сверяются только ОДНОРОДНЫЕ пары, и обе дают ответ сами по себе:
     *
     *   1) `price_amount` против суммы заказа — но лишь когда поставщик прямо
     *      назвал `price_currency`, и она совпадает с нашей валютой заказа.
     *   2) отношение `actually_paid / pay_amount` — здесь валюта сокращается,
     *      поэтому способ верен для любой криптовалюты и не требует догадок.
     *
     * ПОЧЕМУ FAIL-OPEN. Не удалось сверить ни одним способом — платёж НЕ
     * блокируется: деньги уже у нас, и отказ выдать оплаченное хуже, чем
     * выдать непроверенное. Такой случай уходит в журнал и в письмо
     * Архитектору, чтобы его разобрал человек.
     *
     * ДОПУСК — один процент. Криптоплатёж почти никогда не приходит копейка
     * в копейку: комиссия сети и округление курса дают сотые доли.
     */
    {
      const ДОЛЯ_ДОПУСКА = 0.99;   // принимаем от 99 % ожидаемого
      const валютаЗаказа = String(body?.price_currency || '').toLowerCase();
      const ценаЗаказа = Number(body?.price_amount);
      const кОплате = Number(body?.pay_amount);
      const пришло = Number(body?.actually_paid);

      let недоплата: string | null = null;
      let сверено = false;

      // Способ 1 — обе величины в валюте заказа. Наши заказы в долларах.
      if (валютаЗаказа === 'usd' && Number.isFinite(ценаЗаказа) && ценаЗаказа > 0) {
        сверено = true;
        if (ценаЗаказа < amount * ДОЛЯ_ДОПУСКА) {
          недоплата = `в уведомлении заказано $${ценаЗаказа}, `
            + `а в нашей записи $${amount}`;
        }
      }

      // Способ 2 — отношение пришедшего к назначенному. Валюта сокращается,
      // поэтому способ верен для любой монеты и не зависит от курса.
      if (Number.isFinite(кОплате) && кОплате > 0 && Number.isFinite(пришло)) {
        сверено = true;
        if (пришло < кОплате * ДОЛЯ_ДОПУСКА) {
          const доля = (пришло / кОплате * 100).toFixed(1);
          недоплата = `пришло ${пришло} из ${кОплате} — это ${доля} % `
            + `назначенной суммы`;
        }
      }

      if (недоплата) {
        // Снимаем отметку «оплачено», поставленную строкой выше, и зовём
        // человека. Ничего не удаляем — только возвращаем прежний вид.
        await pool.query(
          `UPDATE pay_orders SET status='underpaid', paid_at=NULL WHERE order_id=$1`,
          [orderId]);
        await pool.end();
        console.warn(`[pay/ipn] заказ ${orderId}: НЕДОПЛАТА — ${недоплата}`);
        await alertOwner('НЕДОПЛАТА по заказу',
          `Заказ ${orderId} (тариф ${tier}, $${amount}): ${недоплата}.
...
`
      ...
      if (!сверено) {
        console.warn(`[pay/ipn] заказ ${orderId}: сверить сумму нечем — `
          + `выдаём по заказу ($${amount})`);
        await alertOwner('Оплата выдана без сверки суммы',
