...
export default function Home() {
  ...
  return (
    <div className="min-h-[100svh] w-full overflow-x-hidden flex flex-col bg-[#050507]">
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  {/* Ссылка на политику обязана быть на каждой странице: этого
                      требуют и GDPR, и здравый смысл. Наш же AIfaFocus поймал её
                  ...
                  {/* Юридические сведения обязаны быть доступны с любой
                      ...
                      Наш собственный AIfaFocus отметил их отсутствие здесь. */}
                  ...
                  {/* 🔴 ПОЛИТИКА ВЕЛА НА ЧУЖОЙ ДОМЕН, ХОТЯ СВОЯ ЕСТЬ (09.09.2026).
                      Замер: `radiocode.space/privacy-policy` отвечает 200 — своя
                      страница написана и работает. А подвал уводил на центральный
                      сайт. Довод тот же, что записан рядом про оферту: документ
                      должен быть НА ТОМ САЙТЕ, где человек нажимает кнопку. Здесь
                      есть кабинет и чат, то есть данные собираются именно тут. */}
                  <a
                    href="/privacy-policy"
                  ...
                  {/* ПОЛИТИКА НАШЕГО КРАУЛЕРА. Добавлена 11.09.2026 по прямому
                      указанию Архитектора: ссылка должна быть в меню на ВСЕХ
                      четырёх сайтах, а не только там, где лежит страница.

                      Вебмастер, чей сайт мы обошли, видит в своих логах строку
                      AIfaWorksBot/1.0 (+https://aifa.works/bot) и идёт по ней
                      выяснять, кто его сканировал. Своей копии здесь НЕ
                      заводим сознательно: один документ по одному адресу,
                      иначе четыре версии разойдутся и вебмастер прочитает
                      устаревшие правила. */}
                  <a
      ...
      {/* ПРАЙС AIfaFocus УБРАН С ГЛАВНОЙ РАДИО 11.09.2026 по прямому
          слову Архитектора: «Какого хера посреди сайта радио делает
          прайс от AIfaFocus и на английском?! убрать это дерьмо с
          главной страницы сайта радио прямо сейчас».

          Блок стоял ПОСЛЕ футера и показывал восемь карточек с ценами
          $149–$3 500 по-английски даже при выбранном русском языке.
          Радио — сайт про музыку и эфир; прайс аудита доступности
          здесь не к месту вдвойне.

          НИЧЕГО НЕ ПОТЕРЯНО: прайс живёт на своей странице
