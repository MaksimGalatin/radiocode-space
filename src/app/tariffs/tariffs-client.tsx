      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-12">
        ...
        <div className="mb-16 grid gap-5 md:grid-cols-3">
          {т.тарифы.map((тариф) => (
            ...
            >
              <div className="mb-2.5 text-[15px] font-semibold text-cyan-700 dark:text-cyan-400">
                {тариф.имя}
              </div>
              ...
              {/* Чем отличается от предыдущего уровня и что человек получает
                  на руки. Добавлено 06.09.2026: «разница между ними должна
                  быть полностью очевидна и оправдана». */}
              <p
                className="text-blue-900 dark:text-blue-100"
                style={{
                  fontSize: 13.5, lineHeight: 1.6, marginBottom: 12,
              ...
              <p className="text-slate-700 dark:text-slate-200" style={{ fontSize: 14, lineHeight: 1.6, marginBottom: 16, fontWeight: 600 }}>
                {тариф.получаете}
              </p>
              <ul className="m-0 mb-6 list-none space-y-2 p-0 flex-grow">
                {тариф.входит.map((пункт) => (
                  <li key={пункт} className="relative pl-5 text-sm leading-relaxed text-white/60">
                    <span className="absolute left-0 text-cyan-700 dark:text-cyan-400">·</span>
                    {пункт}
                  </li>
                ))}
              </ul>
        ...
        {/* ПРАВОВАЯ ОГОВОРКА. Добавлена 07.09.2026.
            Страница называет проценты дохода от привлечённых
            людей и выплаты в токене. Без этой оговорки она по
            тесту Хауи читается как предложение ценной бумаги.
            Текст не выдуман: он уже стоял на /ambassador. */}
        {/* Цвет задан КЛАССОМ, а не значением: фон этой страницы меняется
            вместе с темой, и один оттенок не может пройти по контрасту в обеих.
            Замер 10.09.2026: этот текст на белом давал 3.01 при норме 4.5 —
            почти не читался в светлой теме, хотя в тёмной был правилен.
            slate-600 на белом даёт 7.58, slate-400 на тёмном — 7.86. */}
        <p className="text-slate-600 dark:text-slate-400"
           style={{ fontSize: 13, lineHeight: 1.7,
                     marginBottom: 48, maxWidth: 720,
                     borderLeft: '2px solid #35507a', paddingLeft: 14 }}>
          {т.амбассадорыОговорка}
        </p>
        ...
        {/*
          ...
          */}
          ...
          <div className="mb-12 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
            <p className="mb-4 max-w-3xl leading-relaxed text-gray-700 dark:text-gray-300">{т.паспортЧто}</p>
            <ul className="mb-4 max-w-3xl list-none space-y-2 p-0">
              {т.паспортСтроки.map((строка) => (
                <li key={строка} className="relative pl-5 leading-relaxed text-gray-700 dark:text-gray-300">
                  <span className="absolute left-0 text-cyan-700 dark:text-cyan-400">·</span>
                  {строка}
                </li>
              ))}
            </ul>
            <p className="mb-3 max-w-3xl text-sm leading-relaxed text-gray-600 dark:text-gray-400">{т.паспортЦепь}</p>
            <p className="max-w-3xl text-sm leading-relaxed text-gray-600 dark:text-gray-400">{т.паспортДальше}</p>
          </div>
        ...
        <div className="mb-12 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
          ...
          <p className="mb-5 max-w-3xl text-sm leading-relaxed text-white/60">
            {т.оракулЗамерНачало}
            <strong className="text-cyan-700 dark:text-cyan-400">{т.оракулЗамерВыделено}</strong>
            {т.оракулЗамерКонец}
          </p>
          <a
            href="https://aifa.works/accessibility"
            className="text-[15px] text-cyan-700 dark:text-cyan-400 underline"
            rel="noopener"
          >
            {т.оракулСсылка}
          </a>
        ...
        <div className="border-t border-white/10 pt-6">
          <Link href="/cabinet" className="text-base text-cyan-700 dark:text-cyan-400 underline">
            {т.вКабинет}
          </Link>
          ...
          */}
          <div style={{ marginTop: 22, paddingTop: 18, borderTop: '1px solid rgba(148,163,184,0.15)' }}>
            <div className="text-slate-700 dark:text-slate-200" style={{ fontSize: 15, fontWeight: 700, marginBottom: 10 }}>
              {т.лимитыЗаголовок}
            </div>
            <ul className="text-slate-600 dark:text-slate-400" style={{ margin: 0, paddingLeft: 20, fontSize: 14, lineHeight: 1.9 }}>
              {т.лимитыСтроки.map((с) => (
                <li key={с}>{с}</li>
              ))}
            </ul>
            <div className="text-slate-700 dark:text-slate-200" style={{ fontSize: 15, fontWeight: 700, marginTop: 18, marginBottom: 8 }}>
              {т.партнёрскийЗаголовок}
            </div>
            <p className="text-slate-600 dark:text-slate-400" style={{ fontSize: 14, lineHeight: 1.7, margin: 0, maxWidth: 720 }}>
              {т.партнёрскийТекст}
            </p>
          </div>
        </div>
