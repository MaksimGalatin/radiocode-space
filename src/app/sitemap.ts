...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...
    {
      // 🔴 ЗДЕСЬ БЫЛА МОЯ ПОРЧА (починено 10.09.2026).
      //
      // Добавляя заявление о доступности, я вписала его вторым аргументом
      // ВНУТРЬ вызова `языковые('/glossary', …)` — вместо отдельной записи
      // карты. Получилось `Expected 1 arguments, but got 2`: типы падали,
      // сборка Vercel не проходила, и сайт не обновлялся с 17 часов.
      //
      // Заметила это `tsc --noEmit` при сплошной проверке четырёх сайтов —
      // то есть перебором, а не там, где правила.
      //
      // Сама страница: ссылка на неё стояла со страницы сканера и отдавала
      // 404 — заявления не было ни на одном сайте, кроме aifa.works.
    ...
    // 🔴 ДВУХ СТРАНИЦ ИССЛЕДОВАНИЯ В КАРТЕ НЕ БЫЛО. Добавлено 12.09.2026.
    //
    // Обе существуют и отвечают 200 (проверено запросом к живому сайту), обе
    // есть в коде рядом с остальными — но в карту не попали. Замер по четырём
    // сайтам: на центральном, aifa.works и aifa.digital все шесть страниц
    // исследования в карте есть, здесь не хватало ровно этих двух.
    //
    // Отсутствие в карте не закрывает страницу для человека, но поисковик
    // находит её позже и хуже: карта — это то, по чему он обходит сайт.
    {
      // Коммерческие сайты США: та же мера, другая совокупность.
      url: `${SITE}/research/commercial`,
      lastModified: now,
      changeFrequency: 'monthly',
      priority: 0.7,
      alternates: { languages: языковые('/research/commercial') },
    },
    {
      // Десять стран Европы — вторая часть исследования.
      url: `${SITE}/research/europe`,
      lastModified: now,
      changeFrequency: 'monthly',
      priority: 0.7,
      alternates: { languages: языковые('/research/europe') },
    },
