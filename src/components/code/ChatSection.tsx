          ...
          {(voice.listening || voice.error) && (
            <div className="px-4 md:px-6 pb-2 text-xs" style={{ color: voice.error ? "#f87171" : "#22d3ee" }}>
              {(() => { const V: Record<string, [string,string,string,string]> = {
                  ...
                  "no-audio": ["Микрофон не открылся — возможно, его занял другой звонок или программа. Закрой их и попробуй снова.", "The microphone didn't open — another call or app may be holding it. Close them and try again.", "El micrófono no se abrió — otra llamada o app puede estar usándolo. Ciérralas e inténtalo.", "麦克风未能开启——可能被其他通话或程序占用。请关闭后重试。"],
                  "silent": ["Микрофон открыт, но звука с него нет. Проверь в настройках системы, тот ли вход выбран и не выключен ли он.", "The microphone is open but sending no sound. Check that the right input is selected and not muted.", "El micrófono está abierto pero no envía sonido. Revisa que la entrada correcta esté activa.", "麦克风已开启但没有声音。请检查是否选择了正确的输入设备且未静音。"],
                  network: ["Служба распознавания недоступна — проверь подключение к сети.", "The speech service is unavailable — check your network connection.", "El servicio de voz no está disponible — revisa tu conexión.", "语音识别服务不可用——请检查网络连接。"],
