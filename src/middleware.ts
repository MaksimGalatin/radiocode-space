 ...
 * собственный AIfaFocus ставил за это замечание, и ставил справедливо.
 *
...
function buildCsp(nonce: string): string {
  return [
    ...
    // 🔴 ЭТОЙ СТРОКИ ЗДЕСЬ НЕ БЫЛО, А НА ТРЁХ ДРУГИХ САЙТАХ ОНА ЕСТЬ.
    //
    // Без `form-action` политика не ограничивает, КУДА форма отправляет данные.
    // Если на страницу удастся внедрить разметку — через чужой сценарий,
    // расширение браузера, подменённый ответ по пути, — форма отправит введённое
    // на любой адрес, и ни одна другая директива этого не остановит:
    // `default-src` на отправку форм не распространяется.
    // Сверено 10.08.2026 замером заголовков всех четырёх сайтов: у aifa.digital,
    // aifa.works и codeofdigitaleternity.com — `form-action 'self'`, здесь не
    // было ничего. Правило, у которого есть исключение, не выполняется.
    "form-action 'self'",
...
//
// 🔴 19.08.2026: ОБРАБОТЧИКИ ЗАПРОСОВ (api) ТОЖЕ ИСКЛЮЧЕНЫ.
// Замер: `/api/zzz-random` отдавал 404 без заголовков и 200 с HTML-страницей,
// если в запросе был `Authorization`. Причина: такой запрос не берётся из кэша,
// доходит до приложения, прослойка пропускает его дальше, и приложение рисует
// свою страницу «не найдено» с кодом 200. Снаружи это выглядит как «ручка есть
// и отвечает успехом» — на этом ломается любая автоматическая проверка.
// Ни одна ручка `/api/` заголовок `x-locale` не читает (проверено сплошным
// перебором по всем четырём сайтам), поэтому исключение ничего не ломает.
