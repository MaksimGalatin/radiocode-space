...
const SECRETS = [
  ...
  // ── ФОРМАТЫ НАШИХ СОБСТВЕННЫХ СЕРВИСОВ (добавлено 25.08.2026) ─────────────
  // Страж знал шесть форматов и НЕ знал Vercel, Neon API, Resend, Telegram,
  // Slack, Stripe и AWS. Замер того же дня по Мозгу показал, чем это кончается:
  // ШЕСТЬ разных токенов Vercel лежали в отслеживаемых файлах, и ни один
  // сторож про них не сказал ни слова — просто потому, что такого правила
  // ни у кого не было. Один из шести оказался живым.
  //
  // Класс, а не случай: сторож обязан знать формат КАЖДОГО сервиса, которым мы
  // пользуемся, иначе он охраняет чужие двери и не видит своих.
  [/\b(vcp_|vca_)[A-Za-z0-9]{20,}/, 'токен Vercel — выкладывает и удаляет проекты'],
  [/\bnapi_[A-Za-z0-9]{20,}/, 'ключ API Neon — читает строки подключения ко всем базам'],
  [/\bre_[A-Za-z0-9]{8,}_[A-Za-z0-9]{16,}/, 'ключ Resend — письма от нашего имени'],
  [/\b\d{8,10}:AA[A-Za-z0-9_\-]{30,}/, 'токен бота Telegram'],
  [/\bxox[baprs]-[A-Za-z0-9\-]{20,}/, 'токен Slack'],
  [/\bsk_live_[A-Za-z0-9]{20,}/, 'ключ Stripe (боевой)'],
  [/\bAKIA[A-Z0-9]{16}\b/, 'ключ AWS'],
